...
async def main(port: int, addr: str, max_packets: int, log_file: Path = None):
    ...
    async with websockets.connect(uri) as websocket:
        ...
        file_logger = open("YiboHu_DataLogger.txt","w+")
        ...
        for i in range(max_packets):
            ...
            print(f"{i} total messages logged to local txt file")
...
#    filename = Path(data.log).expanduser()
#    file = filename.open("aaa")
#    file.write("" + "\n")
#    file.flush()
#    file.close()

    try:
        asyncio.run(main(data.port, data.host, data.max_packets, data.log))
